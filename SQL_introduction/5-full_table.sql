...
SET @table_creation = (
    SELECT CONCAT('CREATE TABLE `', TABLE_NAME, '` (', GROUP_CONCAT(CONCAT('`', COLUMN_NAME, '` ', COLUMN_TYPE, COLUMN_NULLABLE, COLUMN_DEFAULT)), ', PRIMARY KEY (`id`)) ENGINE=', TABLE_ENGINE, ' DEFAULT CHARSET=', TABLE_COLLATION) AS statement
